...
svg_files = [
    ("hook_1_revenue_profit.svg", "hook_1_revenue_profit.png", "hook_1"),
    ("hook_2_scaling_margin.svg", "hook_2_scaling_margin.png", "hook_2"),
    ("hook_3_net_profit_check.svg", "hook_3_net_profit_check.png", "hook_3")
]
...
for svg_name, png_name, hook_type in svg_files:
    ...
    if not has_content:
        print(f"[{svg_name}] Error: No content remains after transparency.")
        continue
        
    # Crop the image to bounding box
    # Crop box tuple: (left, upper, right, lower)
    # PIL crop is exclusive for right and lower bounds, so we add 1
    crop_box = (min_x, min_y, max_x + 1, max_y + 1)
    cropped_img = out_img.crop(crop_box)
    new_w, new_h = cropped_img.size
    print(f"[{svg_name}] Cropped bounding box: {crop_box} -> New size: {new_w}x{new_h}")
    
    # Save cropped transparent image
    temp_out = os.path.join(scratch_dir, f"{hook_type}_processed.png")
    cropped_img.save(temp_out, format="PNG")
    
    # Read cropped image bytes and encode to base64
    with open(temp_out, 'rb') as img_f:
        encoded_string = base64.b64encode(img_f.read()).decode('utf-8')
        
    # Open the original SVG and update its content
    svg_path = os.path.join(workspace_dir, svg_name)
    with open(svg_path, 'r', encoding='utf-8') as f_svg:
        svg_content = f_svg.read()
        
    # Update viewBox and width/height in SVG element
    # Matches <svg ... width="1448" height="1086" viewBox="0 0 1448 1086" ...>
    ...
    # Update <image ... width="1448" height="1086" ...>
    ...
    # Replace base64 data
    # Matches href="data:image/png;base64,..." or xlink:href="..."
